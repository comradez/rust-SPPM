...
impl Photon {
    pub fn new(pos: Vector3<f64>, dir: Vector3<f64>, norm: Vector3<f64>, flux: Vector3<f64>) -> Self {
        Self { pos, dir, norm, flux } 
    }
}
...
impl Node {
    fn new(min_pos: Vector3<f64>, max_pos: Vector3<f64>, lchild: Option<Box<Node>>, rchild: Option<Box<Node>>, photon_index: usize) -> Self {
        Self { min_pos, max_pos, lchild, rchild, photon_index }
    }
}
...
impl PhotonCompare {
    fn new(dimension: usize) -> Self { Self { dimension } }
...
impl KDTree {
    ...
    fn intersect(p: &Box<Node>, hitpoint: &HitPoint) -> bool {
        hitpoint.radius >= f64::sqrt(
