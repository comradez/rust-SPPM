...
use crate::ray::Ray;
use crate::matrix::render;
use rand::{thread_rng, Rng};

static PHOTON_NUMBER: u32 = 10000;
static ROUND_NUMBER: u32 = 3;
static SAMPLE_NUMBER: u32 = 1;
...
fn photon_trace(group: &Box<Group>, mut ray: Ray, photon_map: &mut Vec<Photon>) {
    ...
    loop {
        ...
        if let Some(hit) = hit {
            ...
            ray = Ray::new(position, direction, Some(*ray.get_flux()));
        } else {
            break;
        }
    }
        ...
        if let Some(hit) = hit {
            ...
            match material.get_type() {
                &MaterialType::DIFFUSE => {
                    ...
                    kd_tree.search(&mut buffer[x][y], color, hit.get_normal(), 1. / SAMPLE_NUMBER as f64);
                },
        ...
        for x in 0 .. width {
            for y in 0 .. height {
                ...
                for _ in 0 .. SAMPLE_NUMBER {
                    let ray = camera.generate_ray(&Vector2::<f64>::from([
                ...
                if round == 0 {
                    ...
                    if picture[x][y].n + buffer[x][y].n > 0. {
                        let ratio = (picture[x][y].n + photon::ALPHA * buffer[x][y].n) / 
                            (picture[x][y].n + buffer[x][y].n);
                        picture[x][y].radius *= f64::sqrt(ratio);
                        picture[x][y].tau += buffer[x][y].tau;
                        picture[x][y].tau *= ratio;
